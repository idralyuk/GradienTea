...
public class BasicAudioReader {
	...
	public void start() {
		...
		thread = new Thread(new Runnable() {
			...
			protected void readSample(AudioBuffer audioBuffer) {
				...
				while (inputLine.available() < byteBuffer.length) {
					try {
						Thread.sleep(1);
				...
				synchronized (buffers) {
					inputLine.read(byteBuffer, 0, byteBuffer.length);
				}
			...
			public void run() {
				try {
					inputLine.open(DESIRED_FORMAT);
					inputLine.start();

					while (running) {
						AudioBuffer normalizedBuffer = buffers[currentBufferIndex + 1];
						...
						if (currentBufferIndex == buffers.length - 2) {
	...
	public static void compressWithRms(float[] input, float[] output) {
